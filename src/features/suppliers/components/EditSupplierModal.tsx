...
export default function EditSupplierModal({ supplier, onClose, onSaved }: Props) {
  ...
  const [status,    setStatus]    = useState<"active" | "inactive">(supplier.status);
  const [rating,    setRating]    = useState<1|2|3|4|5>(supplier.rating);
  const [debtAmount, setDebtAmount] = useState<number>(supplier.debtAmount);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-900">Chỉnh sửa nhà cung cấp</h2>
            <p className="text-xs text-slate-400 mt-0.5 font-mono">{supplier.id}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
          ...
          {/* Đánh giá */}
          <div className="space-y-1.5">
            <label className={lbl}>Đánh giá</label>
